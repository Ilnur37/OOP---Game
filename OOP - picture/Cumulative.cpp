...
void Cumulative::ShootCumFromLeft(bool& End)
{
	Sleep(10);
	HPEN PenR11 = CreatePen(PS_SOLID, 3, RGB(255, 255, 0));
			...
			while (a1 < 1850) {
				...
				for (int i = 0; i < vector_of_objects.size(); i++)
				{
							...
							(b1 < tempY1 + 8 * tempC1) && (b1 + 10 > tempY1 + 18 * tempC1)))
					{
						...
						vector_of_objects[i]->SetHealth(vector_of_objects[i]->GetHealth() - 62 + vector_of_objects[i]->GetArmor());
...
void Cumulative::ShootCumFromRight(bool& End)
{
	HPEN PenR22 = CreatePen(PS_SOLID, 3, RGB(255, 255, 0));
...
void Cumulative::CrushCum(int x, int y)
{
